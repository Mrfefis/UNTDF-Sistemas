...
implementation
	...
	begin
		...
		begin
			...
			begin
				peregrino:= guia;
				if (guia^.info>info) then
					guia:= guia^.izq
				else
					guia:= guia^.der
			end;
			if (peregrino^.info>info) then
				peregrino^.izq:= nodo
			else
				peregrino^.der:= nodo
		end
	...
	function sucesor(puntero: Tpuntero): Tpuntero;
	begin
		puntero:= puntero^.der;
		while (puntero^.izq<>nil) do
			puntero:= puntero^.izq;
		sucesor:= puntero
	end;
	...
	begin
		peregrino:= nil;
		guia:= arbol;
		while (guia<>nil) and (guia^.info<>info) do
		begin
			peregrino:= guia;
			if (guia^.info>info) then
				guia:= guia^.izq
	...
	function buscar(arbol: Tarbol; info: Tinfo): Tpuntero;
	begin
		while (arbol<>nil) and (arbol^.info<>info) do
			if (arbol^.info>info) then
				arbol:= arbol^.izq
			else
				arbol:= arbol^.der;
		buscar:= arbol;
